  ...
  return (
    <div className="detalhes-container">
      ...
      <div id="detalhes-produto-form">
        ...
        <p>
          <strong><FormattedMessage id="productDetails.status" defaultMessage="Estado:" /></strong>{' '}
          <FormattedMessage id={`productStates.${product.status}`} defaultMessage={product.status} />
        </p>

        {/* 💬 Lógica para redirecionar para perfil correto */}
        {user ? (
          user.username !== product.sellerUsername ? (
            <Link
              to={`/perfil/${product.sellerUsername}`}
            ...
            <Link
              to={`/painel-utilizador/${user.id}`}
              className="seller-profile-link"
              title={formatMessage({ id: 'productDetails.viewSellerProfile', defaultMessage: 'Ver perfil privado' })}
            >
              <i className="fa fa-user" aria-hidden="true"></i>
              <FormattedMessage id="productDetails.viewSellerProfile" defaultMessage="Ver perfil privado" />
            </Link>
          )
        ) : (
          <a
            ...
            onClick={(e) => {
              e.preventDefault();
              alert(formatMessage({
                id: 'productDetails.alert.mustBeLoggedInToViewProfile',
                defaultMessage: 'Só pode aceder a este perfil se for membro da nossa comunidade. Registe-se e/ou faça login!'
              }));
              navigate('/login');
            }}
          >
  ...
  >
    <FormattedMessage id="userProfile.sendMessage" defaultMessage="Enviar mensagem" />
      ...
      <Modal
        isOpen={isModalOpen}
        onClose={closeModal}
        title={<FormattedMessage id="productDetails.modalTitle" defaultMessage="Editar Produto" />}
      >
        <EditProductForm
